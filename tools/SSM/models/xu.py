...
class XuModel(SSMModelTemplate, AbstractSSMModel):
    ...
    def _render_results_trace(cls):
        with st.expander("📐 Full formula trace — Xu's T (2014)", expanded=False):
            st.markdown("**Dependency chain:** Y_ex1 → peel Ybcx (= 1/Rbcx + jωCbcx) → Z_in → intrinsic")
            st.markdown("**Step 2** *(input: Y_ex1; Rbcx defaulted to 285 kΩ)*")
            ...
            st.markdown("**Step 3** *(input: Y_ex1, Cbcx, Rbcx)*")
