...
def _render_topology_illustration(all_p: dict, topology: str, fname: str,
    ...
    smith_png : optional PNG bytes of the user's customized matplotlib Smith
                ...
                column shows the same schematic with that Smith chart composited
                at the bottom-right (placement tunable via SMITH_OVERLAY_* in
                models/_shared.py).
    """
    import io
    try:
    ...
    # Highlight the component the user is currently editing (diagram mode) —
    # a red ring centred on that component's value-label position, drawn first
    # so the value text renders on top.
    ...
    # Default: single schematic.  When the model has no parasitics and a Smith
    # data set is supplied, show a second column with the Smith overlaid.
    if no_parasitics and smith_png is not None:
        from ._shared import (composite_smith_overlay,
                              PI_SMITH_PAD_FRAC,
                              PI_SMITH_OVERLAY_SIZE_FRAC,
                              PI_SMITH_OVERLAY_POS_FRAC)
        ...
        with c_smith:
            try:
                ...
                # (controls live in models/_shared.py: PI_SMITH_*).
                if topology == "pi":
                    img2 = composite_smith_overlay(
                        img, smith_png, pad=PI_SMITH_PAD_FRAC,
                        size_frac=PI_SMITH_OVERLAY_SIZE_FRAC,
                        pos_frac=PI_SMITH_OVERLAY_POS_FRAC)
                else:
...
class ChengT(SSMModelTemplate, AbstractSSMModel):
    ...
    def _render_topology(cls, all_p, fname, smith_png=None, highlight_key=None):
        _render_topology_illustration(all_p, "T", fname, smith_png=smith_png,
...
class ChengPi(SSMModelTemplate, AbstractSSMModel):
    ...
    @classmethod
    def _render_topology(cls, all_p, fname, smith_png=None, highlight_key=None):
        _render_topology_illustration(all_p, "pi", fname, smith_png=smith_png,
                                      highlight_key=highlight_key)
