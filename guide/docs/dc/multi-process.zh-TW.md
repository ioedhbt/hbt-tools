...
1. 將 B1500A CSV 匯出檔拖放到上傳區。可一次上傳多個檔案，每個檔案上限 350
   MB，僅接受 CSV。
...
**Other**，且不會進一步處理，請依此規則命名匯出檔，或在上傳前重新命名。

## 結果

2. 每個能辨識的檔案都會立即轉換，不需按任何按鈕。畫面會出現兩個 ZIP
   ...
   `Gummel.xlsx`、`TLM.xlsx`、`Other.xlsx`。工具無法分類的檔案（前綴不是
   `BC`/`BE`/`Fa`/`Gu`/`Tl`）仍會出現在這裡，歸在 **Other** 底下，在上面這次
   執行中，批次裡有一個 `TEST Family […] - Copy.csv` 不符合任何前綴，因此被
   算作略過，而不是誤判為 Family 曲線。

## 交給 DC 分析

3. 點選 **🔬 在 DC 分析中檢視 (Analyze Data in DC Analysis)**，將
...
- **E5270B CITI 檔案工具 (E5270B citi File Tool)**：接受 `.citi`、`.txt`
  與純 `.csv`。這是 CITI 格式的處理路徑：它會解析 `VAR`/`DATA` 區塊標頭
  （一個 `VAR` 區塊 → 二極體，兩個 → 族群），或是純三欄的
  Vb/Ib/Ic 文字傾印，並和 CSV 工具一樣把結果歸類成 BE/BC/Gummel/Family。
