...
can give a negative τC, which is not physical; overwrite τB and τC with the
values from the [transit-time fit](ssm-transit-time.md), which is measured
across the whole bias sweep rather than split out of one point.
...
**S display scale** multiplies each trace before plotting, a way to pull
S12 up to a readable size next to S21. Display only; it does **not** change
the residual.

**Total residual** is the fit error over the whole band, with a breakdown
per S-parameter.
...
- S12 is almost always the worst of the four; it is the smallest signal and
  the most affected by leftover pad capacitance.
- One parameter far above the others points at a specific element, not at
  the model. S22 high means the collector side (Rc, Cbc); S11 high means
  the base side.
...
Measured traces are solid with markers, modelled traces are dashed. Read
them together: the Smith chart shows where in the band the model departs,
the gain plot shows what that costs you in f_T and f_max.
...
- **f_max**, from the fitted model. The extrapolated value on the gain plot
  is the measured counterpart; they should agree within a few percent.
...
2. **💾 Fit cache** shows what has been saved for this device and model.
