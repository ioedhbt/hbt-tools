...
載入元件並選擇小訊號模型拓樸。本頁示範如何使用你自己的原始（未去嵌入）
檔案，從空白上傳走到完成去嵌入的元件，逐步操作 SSM 萃取頁。
...
在**① 上傳偏壓檔 (① Upload bias files)** 下上傳一個或多個原始偏壓
   `.s2p` 檔案。使用仍保留焊墊與引線的原始偏壓檔。
...
在側邊欄開啟**啟用元件 dummy 去嵌入 (Enable device-dummy de-embedding)**。

![開啟側邊欄開關](../assets/ssm/setup_sidebar_toggle.png)

畫面會出現兩個檔案欄位：**元件 Open (Dev Open)** 與**元件 Short
(Dev Short)**。將你的 Open 檔案拖入第一個，Short 檔案拖入第二個。
...
在**② 選擇元件與模型 (② Select device & model)** 下選擇使用中的檔案
...
表格應給出 Cpbe、Cpce 和 Cpbc 的值。如有必要，將頻率範圍滑桿移到 Open
dummy 最乾淨的區段。
