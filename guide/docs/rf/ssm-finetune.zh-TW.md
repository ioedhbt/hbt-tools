...
渡越時間就是在這裡修正。Cheng 的解析萃取可能給出負的 τC，並不物理；請
改用[渡越時間擬合](ssm-transit-time.md)得到的 τB 與 τC 覆寫它們，該擬合
是跨整組偏壓掃描量測而來，而非由單一萃取拆分。
...
**Total residual** 是整個頻段上的擬合誤差，並附上各 S 參數的細項。
