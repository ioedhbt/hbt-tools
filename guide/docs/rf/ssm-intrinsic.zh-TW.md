...
低頻估計值取自量測雜訊最大的區段。以此元件為例，低頻值為 484.3 fF，
中位數為 481 fF，高頻值為 578.9 fF。中位數會忽略頻段兩端的振鈴現象，
在整個偏壓掃描中是較為穩定可重複的選擇。在此設定好之後再繼續；後面每個
依賴 Cbc 的步驟都會沿用數值框中的內容。
...
τC 是負值，而真實的集極不會如此。這是把單一解析萃取拆分成兩項所產生的
假象，也是為什麼[渡越時間擬合](ssm-transit-time.md)才是 τB 與 τC 更好的
來源。出貨模型前，請用該處的值取代這兩個參數。

!!! tip
    `cold = 230.2 fF` 晶片會改用冷態量測值填入數值框。那是零偏壓下的
    接面電容，而非有偏壓時的值，因此應將其視為健全性檢查，而非可直接
    採用的數值。
