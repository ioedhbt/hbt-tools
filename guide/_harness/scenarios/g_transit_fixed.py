...
def run(page, shot):
    ...
    p("skipping ic-table shot; already captured")
    ...
    # The Inputs row is Re-REE, Rc, Cbc total, eta, T in DOM order. Anchoring
    # on the label text is unreliable (each carries a help-icon sibling), so
    # index off the section heading instead.
    for idx, value in ((1, "64.3966"), (2, "19.0178"), (3, "484.2610")):
        box = fit_hdr.locator(f"xpath=following::input[{idx}]")
        p("input", idx, "was", box.input_value())
        box.fill(value)
        box.press("Tab")
