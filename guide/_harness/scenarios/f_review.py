"""ssm-finetune.md — Section 5 (Review): fine-tune, residual, Smith, handoff.

Derived from b_intrinsic_final.py, which is the recipe that reliably reaches
Section 4 on this page.  Viewport clips only — full_page stitches the whole
document and stops matching viewport-relative bounding boxes after a scroll.
"""
import sys
...
def run(page, shot):
    ...
    page.evaluate(_HIDE_CSS)
    print("REVIEW BOX", head.bounding_box(), flush=True)
    shot("00_review_top")
